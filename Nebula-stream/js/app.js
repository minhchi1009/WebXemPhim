...
// Import các icon (Lúc này chưa có fallback an toàn)
const { Play, Info, Plus, Search, Bell, X, Maximize, Volume2, Star, TrendingUp, ChevronRight, Menu, LogOut, Calendar, Clock, ThumbsUp } = Icons;

// --- COMPONENT: MOVIE DETAIL MODAL (Phiên bản ban đầu) ---
const MovieDetailModal = ({ movie, onClose, onPlay }) => {
    ...
    return (
        // Lúc này z-index chỉ là 60 (có thể bị thấp hơn một số thành phần khác)
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 animate-fade-in">
            ...
            {/* Hộp nội dung */}
            <div 
                className="relative w-full max-w-4xl bg-[#181818] rounded-2xl overflow-hidden shadow-2xl animate-fade-in-up max-h-[90vh] overflow-y-auto scrollbar-hide ring-1 ring-white/10"
                onClick={handleContentClick}
            >
                <button 
                    onClick={onClose} 
                    className="absolute top-4 right-4 z-20 bg-[#181818] p-2 rounded-full hover:bg-white/20 transition text-white"
                >
                    {X && <X size={24} />}
                </button>
                    ...
                    <div className="absolute bottom-8 left-8 right-8">
                        ...
                        <div className="flex items-center gap-4">
                            <button 
                                ...
                                className="flex items-center gap-2 px-8 py-3 bg-white text-black font-bold rounded hover:bg-gray-200 transition transform hover:scale-105"
                            >
                                {Play && <Play size={24} fill="currentColor" />} Phát
                            </button>
                            <button className="flex items-center gap-2 px-4 py-3 bg-gray-600/60 border border-gray-500 text-white font-bold rounded hover:bg-gray-600/80 transition">
                                {Plus && <Plus size={24} />} Danh sách
                            </button>
                ...
                <div className="p-8 grid grid-cols-1 md:grid-cols-3 gap-8 text-white">
                    <div className="md:col-span-2 space-y-6">
                        <div className="flex items-center gap-4 text-sm font-bold flex-wrap">
                            <span className="text-green-400 font-extrabold">{movie.match || "98% Match"}</span>
                            <span className="text-gray-400">{movie.year || "2024"}</span>
                            <span className="border border-gray-500 px-1.5 py-0.5 text-xs rounded-sm bg-white/10">{movie.age || "16+"}</span>
                            <span className="text-gray-400">{movie.duration || "2h 15m"}</span>
                            <span className="border border-white/40 px-1 text-[10px] rounded flex items-center">HD</span>
                        </div>
                        
                        <p className="text-base md:text-lg text-gray-200 leading-relaxed">
                            {movie.description || "Một tác phẩm điện ảnh đầy kịch tính, đưa người xem vào một hành trình không thể đoán trước. Với kỹ xảo mãn nhãn và cốt truyện sâu sắc, đây là bộ phim bạn không thể bỏ lỡ trong năm nay."}
                        </p>
                    </div>
                    ...
                    <div className="text-sm space-y-4">
                        <div>
                            ...
                            <span className="text-gray-300">Keanu Reeves, Scarlett Johansson</span>
                        </div>
                        <div>
                            <span className="text-gray-500 block mb-1">Thể loại:</span>
                            <span className="text-gray-300">Hành động, Sci-Fi</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 flex items-center justify-between">
                ...
                <div className="flex items-center gap-5 text-gray-300">
                    <div className="hidden md:flex items-center bg-white/5 rounded-full px-3 py-1.5 backdrop-blur-sm border border-white/5 focus-within:border-cyan-500/50 transition-all group hover:bg-white/10">
                        ...
                        <input type="text" placeholder="Tìm kiếm..." className="bg-transparent border-none outline-none text-sm ml-2 w-24 group-focus-within:w-48 transition-all duration-300 text-white placeholder-gray-500" />
                    </div>
                    ...
                    {username ? (
                        ...
                        <div onClick={() => window.location.href = 'login.html'} className="w-9 h-9 rounded-full bg-white/10 p-[2px] cursor-pointer hover:bg-white/20 border border-white/20 transition-all flex items-center justify-center group" title="Đăng nhập">
...
const Hero = ({ onPlay, onOpenDetail }) => {
    return (
        <div className="relative w-full h-[85vh] md:h-screen overflow-hidden">
        <div className="absolute inset-0">
            <img src={HERO_MOVIE.image} alt="Hero Background" className="w-full h-full object-cover animate-pan-slow" />
        ...
        <div className="relative z-10 h-full flex items-center px-4 md:px-12 max-w-7xl mx-auto pt-20">
            ...
            <h1 className="text-5xl md:text-7xl font-black text-white leading-tight drop-shadow-2xl">{HERO_MOVIE.title}</h1>
            ...
            <p className="text-gray-300 text-lg leading-relaxed line-clamp-3 md:line-clamp-none max-w-xl text-shadow-sm">{HERO_MOVIE.description}</p>
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
                <button onClick={onPlay} className="group flex items-center justify-center gap-3 px-8 py-4 bg-white text-black font-bold rounded-xl hover:scale-105 transition-all duration-300 shadow-[0_0_20px_rgba(255,255,255,0.3)] hover:shadow-[0_0_30px_rgba(255,255,255,0.5)]">
                    {Play && <Play className="fill-black group-hover:fill-purple-600 group-hover:text-purple-600 transition-colors" size={20} />}
                    <span>Xem Ngay</span>
                </button>
                {/* Nút Thông tin gọi hàm onOpenDetail */}
                <button onClick={() => onOpenDetail(HERO_MOVIE)} className="flex items-center justify-center gap-3 px-8 py-4 bg-white/10 text-white font-bold rounded-xl backdrop-blur-md border border-white/20 hover:bg-white/20 transition-all duration-300">
                    {Info && <Info size={20} />}
                    <span>Thông Tin</span>
                </button>
            </div>
...
// MovieCard dùng onClick (chưa đổi thành onSelect)
const MovieCard = ({ movie, onClick }) => (
    ...
    >
        <img src={movie.image} alt={movie.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:brightness-110" />
                ...
                <div className="flex items-center justify-between text-xs text-gray-300">
                    ...
                    <div className="flex gap-2">
                        <div className="p-1.5 bg-white rounded-full text-black hover:bg-cyan-400 transition-colors">
                            {Icons.ChevronDown ? <Icons.ChevronDown size={10} /> : <div className="w-2 h-2"></div>}
                        </div>
                    </div>
                </div>
            </div>
...
// MovieRow nhận prop onMovieClick và truyền xuống Card
...
const App = () => {
    ...
    // STATE MỚI: Theo dõi phim đang được chọn để xem chi tiết
    ...
    return (
        <div className="min-h-screen bg-[#0b0c15] text-white font-sans selection:bg-cyan-500/30 selection:text-cyan-100">
            ...
            <main>
                {/* Hero truyền thêm hàm mở chi tiết */}
                <Hero onPlay={handlePlay} onOpenDetail={openMovieDetail} />
                ...
                {CATEGORIES.map((cat, index) => (
                    <MovieRow 
                        ...
                        onMovieClick={openMovieDetail} // Truyền hàm click vào từng phim
                    />
                ))}
                </div>
            </main>
